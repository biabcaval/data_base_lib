...
from db_config import get_mysql_connection, app
...
@app.route('/api/search_livro', methods=['GET'])
def search():
    conn = None
    cursor = None
    try:
...
@app.route('/api/delete_livro', methods=['DELETE'])
def delete():
    conn = None
    cursor = None
    try:
...
@app.route('/api/update_livro/<int:id_livro>', methods=['PUT'])
...
#app.register_blueprint(app,url_prefix='/livros')
